        ...
        while !input.is_empty() {
            ...
            for (i, cons) in consequences_tokens.into_iter().enumerate() {
                if i == 0 {
                    ...
                    consequences_tokens_punct.extend(quote_spanned! { cons.span() => punct.push(#crate_path::ast::Comma{ span: #crate_path::ast::Span::new(#from_str, ",") }, #cons); });
                }
            }
            let consequences_tokens: TokenStream2 = quote_spanned! { first_span => { let mut punct = #crate_path::ast::Punctuated::new(); #consequences_tokens_punct punct } };
            ...
            let antecedents_tokens: TokenStream2 = if let Some(((colon, _), antecedents)) = antecedents {
                ...
                for (i, ant) in antecedents.into_iter().enumerate() {
                    if i == 0 {
                        ...
                        antecedents_tokens.extend(quote_spanned! { ant.span() => punct.push(#crate_path::ast::Comma{ span: #crate_path::ast::Span::new(#from_str, ",") }, #ant); });
                    }
                }
                let antecedents_tokens: TokenStream2 = quote_spanned! { first_span => { let mut punct = #crate_path::ast::Punctuated::new(); #antecedents_tokens punct } };
